...
impl CallValueApi for AndesApiImpl {
    ...
    fn get_all_dcdt_transfers(
        &self,
    ) -> testnumbat_wasm::types::ManagedVec<Self, testnumbat_wasm::types::DcdtTokenPayment<Self>> {
        unsafe {
            let result_handle = mBufferNew();
            managedGetMultiDCDTCallValue(result_handle);
            testnumbat_wasm::types::ManagedVec::from_raw_handle(self.clone(), result_handle)
        }
    }
}
