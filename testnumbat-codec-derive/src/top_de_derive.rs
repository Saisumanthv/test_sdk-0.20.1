    ...
    match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            let top_decode_body = quote! {
                ...
                if !testnumbat_codec::NestedDecodeInput::is_depleted(&nested_buffer) {
                    return core::result::Result::Err(testnumbat_codec::DecodeError::INPUT_TOO_LONG);
                }
            ...
            let top_decode_or_exit_body = quote! {
                ...
                if !testnumbat_codec::NestedDecodeInput::is_depleted(&nested_buffer) {
                    exit(c, testnumbat_codec::DecodeError::INPUT_TOO_LONG);
                }
                result
            };
        ...
        syn::Data::Enum(data_enum) => {
            ...
            if is_fieldless_enum(data_enum) {
                ...
                let top_decode_body = quote! {
                    match <u8 as testnumbat_codec::TopDecode>::top_decode(top_input)? {
                        #(#top_decode_arms)*
                        _ => core::result::Result::Err(testnumbat_codec::DecodeError::INVALID_VALUE),
                    }
                };
                let top_decode_or_exit_body = quote! {
                    match <u8 as testnumbat_codec::TopDecode>::top_decode_or_exit(top_input, c.clone(), exit) {
                        #(#top_decode_or_exit_arms)*
                        _ => exit(c, testnumbat_codec::DecodeError::INVALID_VALUE),
                    }
                };
                (top_decode_body, top_decode_or_exit_body)
            } else {
                ...
                let top_decode_body = quote! {
                    let mut nested_buffer = top_input.into_nested_buffer();
                    let result = match <u8 as testnumbat_codec::NestedDecode>::dep_decode(&mut nested_buffer)? {
                ...
                let top_decode_or_exit_body = quote! {
                    let mut nested_buffer = top_input.into_nested_buffer();
                    let result = match <u8 as testnumbat_codec::NestedDecode>::dep_decode_or_exit(&mut nested_buffer, c.clone(), exit) {
...
pub fn top_decode_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics testnumbat_codec::TopDecode for #name #ty_generics #where_clause {
            fn top_decode<I: testnumbat_codec::TopDecodeInput>(top_input: I) -> core::result::Result<Self, testnumbat_codec::DecodeError> {
                #auto_default
                #top_decode_body
            }

            fn top_decode_or_exit<I: testnumbat_codec::TopDecodeInput, ExitCtx: Clone>(
                top_input: I,
                c: ExitCtx,
                exit: fn(ExitCtx, testnumbat_codec::DecodeError) -> !,
            ) -> Self {
...
pub fn top_decode_or_default_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics testnumbat_codec::TopDecode for #name #ty_generics #where_clause {
            fn top_decode<I: testnumbat_codec::TopDecodeInput>(top_input: I) -> core::result::Result<Self, testnumbat_codec::DecodeError> {
                if top_input.byte_len() == 0 {
                    Ok(<#name #ty_generics as testnumbat_codec::DecodeDefault>::default())
                } else {
                    #top_decode_body
                }
            }
            ...
            fn top_decode_or_exit<I: testnumbat_codec::TopDecodeInput, ExitCtx: Clone>(
                ...
                if top_input.byte_len() == 0 {
                    <#name #ty_generics as testnumbat_codec::DecodeDefault>::default()
                } else {
                    #top_decode_or_exit_body
                }
            }
