...
pub fn dep_decode_snippet(
    ...
    if let Some(ident) = &field.ident {
        quote! {
            #ident: <#ty as testnumbat_codec::NestedDecode>::dep_decode(#input_value)?
        }
    } else {
        quote! {
            <#ty as testnumbat_codec::NestedDecode>::dep_decode(#input_value)?
        }
    }
}
...
pub fn dep_decode_or_exit_snippet(
    ...
    if let Some(ident) = &field.ident {
        quote! {
            #ident: <#ty as testnumbat_codec::NestedDecode>::dep_decode_or_exit(#input_value, c.clone(), exit)
        }
    } else {
        quote! {
            <#ty as testnumbat_codec::NestedDecode>::dep_decode_or_exit(#input_value, c.clone(), exit)
        }
    }
}
...
pub fn nested_decode_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            quote! {
                impl #impl_generics testnumbat_codec::NestedDecode for #name #ty_generics #where_clause {
                    fn dep_decode<I: testnumbat_codec::NestedDecodeInput>(input: &mut I) -> core::result::Result<Self, testnumbat_codec::DecodeError> {
                        core::result::Result::Ok(
                            #name #field_dep_decode_snippets
                        )
                    }

                    fn dep_decode_or_exit<I: testnumbat_codec::NestedDecodeInput, ExitCtx: Clone>(
                        input: &mut I,
                        c: ExitCtx,
        ...
        syn::Data::Enum(data_enum) => {
            ...
            quote! {
                impl #impl_generics testnumbat_codec::NestedDecode for #name #ty_generics #where_clause {
                    fn dep_decode<I: testnumbat_codec::NestedDecodeInput>(input: &mut I) -> core::result::Result<Self, testnumbat_codec::DecodeError> {
                        match <u8 as testnumbat_codec::NestedDecode>::dep_decode(input)? {
                    ...
                    fn dep_decode_or_exit<I: testnumbat_codec::NestedDecodeInput, ExitCtx: Clone>(
                        ...
                        match <u8 as testnumbat_codec::NestedDecode>::dep_decode_or_exit(input, c.clone(), exit) {
