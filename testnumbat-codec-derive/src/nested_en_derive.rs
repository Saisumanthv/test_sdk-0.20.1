...
pub fn dep_encode_snippet(value: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    quote! {
        testnumbat_codec::NestedEncode::dep_encode(&#value, dest)?;
    }
}

pub fn dep_encode_or_exit_snippet(value: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    quote! {
        testnumbat_codec::NestedEncode::dep_encode_or_exit(&#value, dest, c.clone(), exit);
    }
}
...
pub fn nested_encode_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            quote! {
                impl #impl_generics testnumbat_codec::NestedEncode for #name #ty_generics #where_clause {
                    fn dep_encode<O: testnumbat_codec::NestedEncodeOutput>(&self, dest: &mut O) -> core::result::Result<(), testnumbat_codec::EncodeError> {
                        #(#field_dep_encode_snippets)*
                        core::result::Result::Ok(())
                    }

                    fn dep_encode_or_exit<O: testnumbat_codec::NestedEncodeOutput, ExitCtx: Clone>(
                        &self,
                        dest: &mut O,
                        c: ExitCtx,
