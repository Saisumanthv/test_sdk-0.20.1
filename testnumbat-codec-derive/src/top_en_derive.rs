    ...
    data_enum
        ...
        .map(|(variant_index, variant)| {
            ...
            if variant.fields.is_empty() {
                ...
                quote! {
                    #name::#variant_ident =>
                        testnumbat_codec::TopEncode::top_encode(&#variant_index_u8, output),
                }
            } else {
                ...
                quote! {
                    #name::#variant_ident #local_var_declarations => {
                        ...
                        let dest = &mut buffer;
                        testnumbat_codec::NestedEncode::dep_encode(&#variant_index_u8, dest)?;
    ...
    data_enum
		...
		.map(|(variant_index, variant)| {
			...
			if variant.fields.is_empty() {
				...
				quote! {
					#name::#variant_ident =>
						testnumbat_codec::TopEncode::top_encode_or_exit(&#variant_index_u8, output, c.clone(), exit),
				}
			} else {
				// dep-encode to buffer first
				let local_var_declarations =
				...
				quote! {
					#name::#variant_ident #local_var_declarations => {
						let mut buffer = output.start_nested_encode();
						let dest = &mut buffer;
						testnumbat_codec::NestedEncode::dep_encode_or_exit(&#variant_index_u8, dest, c.clone(), exit);
						#(#variant_field_snippets)*
						output.finalize_nested_encode(buffer);
					},
				}
...
pub fn top_encode_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics testnumbat_codec::TopEncode for #name #ty_generics #where_clause {
            fn top_encode<O: testnumbat_codec::TopEncodeOutput>(&self, output: O) -> core::result::Result<(), testnumbat_codec::EncodeError> {
                #top_encode_body
            }

            fn top_encode_or_exit<O: testnumbat_codec::TopEncodeOutput, ExitCtx: Clone>(
                &self,
                output: O,
                c: ExitCtx,
                exit: fn(ExitCtx, testnumbat_codec::EncodeError) -> !,
            ) {
...
pub fn top_encode_or_default_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics testnumbat_codec::TopEncode for #name #ty_generics #where_clause {
            fn top_encode<O: testnumbat_codec::TopEncodeOutput>(&self, output: O) -> core::result::Result<(), testnumbat_codec::EncodeError> {
                if testnumbat_codec::EncodeDefault::is_default(self) {
                    output.set_slice_u8(&[]);
                    Ok(())
                } else {
                    #top_encode_body
