...
fn print_testdenali_rs(file: &mut File, names: &[String]) {
    for name in names.iter() {
        writeln!(
            file,
            "#[test]
fn {}_rs() {{
    testnumbat_wasm_debug::testdenali_rs(\"testdenali/{}.scen.json\", &contract_map());
}}
",
...
fn print_testdenali_go(file: &mut File, names: &[String]) {
    for name in names.iter() {
        writeln!(
            file,
            "#[test]
fn {}_go() {{
    testnumbat_wasm_debug::testdenali_go(\"testdenali/{}.scen.json\");
}}
",
            name.replace('-', "_").to_lowercase(),
            name
...
/// Examples how to run:
/// `cargo run ../../contracts/examples/erc20/testdenali`
/// `cargo run ../../contracts/feature-tests/basic-features/testdenali`
fn main() {
    ...
    let mut rs_file = File::create("testdenali_rs_test.rs").unwrap();
    print_testdenali_rs(&mut rs_file, names.as_slice());

    let mut go_file = File::create("testdenali_go_test.rs").unwrap();
    print_testdenali_go(&mut go_file, names.as_slice());
}
