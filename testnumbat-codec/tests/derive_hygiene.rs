...
// i.e. any types that are in scope and happen to have the same name as testnumbat-codec types do not break the build.
...
// Also adding all public functions exposed by testnumbat-codec.
...
where
    ST: testnumbat_codec::NestedEncode
        + testnumbat_codec::NestedDecode
        + testnumbat_codec::TopEncode
        + testnumbat_codec::TopDecode,
{
    data: u64,
    trait_stuff: ST,
}
