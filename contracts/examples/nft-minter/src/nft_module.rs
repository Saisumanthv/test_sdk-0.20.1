...
use testnumbat_wasm::testnumbat_codec::TopEncode;
...
pub trait NftModule {
    ...
    fn set_local_roles(&self) -> SCResult<AsyncCall> {
        ...
        Ok(self
            ...
            .set_special_roles(
                ...
                (&[DcdtLocalRole::NftCreate][..]).into_iter().cloned(),
            )
            .async_call())
    }
    ...
    fn require_local_roles_set(&self) -> SCResult<()> {
        let nft_token_id = self.nft_token_id().get();
        let roles = self.blockchain().get_dcdt_local_roles(&nft_token_id);

        require!(
            roles.contains(&DcdtLocalRole::NftCreate),
