...
import { Account, Argument, BackendSigner, Balance, Code, ContractFunction, GasLimit, NetworkConfig, ProxyProvider, SmartContract } from "@testnumbatnetwork/moajs";
import { AbiRegistry, U32Value } from "@testnumbatnetwork/moajs/out/smartcontracts/typesystem";
import { BinaryCodec } from "@testnumbatnetwork/moajs/out/smartcontracts/codec"
...
async function main() {
    ...
    let namespace = abi.findNamespace("lottery-rewa");

    await NetworkConfig.getDefault().sync(provider);
    await user.sync(provider);

    // Deploy TRANSACTION
    let contractFile = path.resolve(__dirname, "../output/lottery-rewa.wasm");
