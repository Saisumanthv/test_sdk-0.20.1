...
pub fn generate_callback_selector_and_main(
    ...
    if let Some(raw) = raw_decl {
        ...
        let cb_selector_body = quote! {
            #as_call_method
            testnumbat_wasm::types::CallbackSelectorResult::Processed
        };
        let cb_main_body = quote! {
            let _ = self.callback_selector(testnumbat_wasm::types::CallbackClosureForDeser::new_empty(self.raw_vm_api()));
        };
        ...
        if match_arms.is_empty() && module_calls.is_empty() {
            let cb_selector_body = quote! {
                testnumbat_wasm::types::CallbackSelectorResult::NotProcessed(___cb_closure___)
            };
            let cb_main_body = quote! {};
            (cb_selector_body, cb_main_body)
        } else {
            ...
            let cb_main_body = quote! {
                if let Some(___cb_closure___) = testnumbat_wasm::types::CallbackClosureForDeser::storage_load_and_clear(self.raw_vm_api()) {
                    if let testnumbat_wasm::types::CallbackSelectorResult::NotProcessed(_) =
                        self::EndpointWrappers::callback_selector(self, ___cb_closure___)	{
                        testnumbat_wasm::api::ErrorApi::signal_error(&self.raw_vm_api(), err_msg::CALLBACK_BAD_FUNC);
                    }
                }
            };
            (cb_selector_body, cb_main_body)
...
fn callback_selector_body(
    ...
    quote! {
        ...
        if ___cb_closure_matcher___.matches_empty() {
            return testnumbat_wasm::types::CallbackSelectorResult::Processed;
        }
        #(#match_arms)*
        #(#module_calls)*
        testnumbat_wasm::types::CallbackSelectorResult::NotProcessed(___cb_closure___)
		...
		.map(|supertrait| {
			...
			quote! {
				match #module_path EndpointWrappers::callback_selector(self, ___cb_closure___) {
					...
					testnumbat_wasm::types::CallbackSelectorResult::NotProcessed(recovered_cb_closure) => {
