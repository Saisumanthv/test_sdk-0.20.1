...
pub fn generate_proxy_endpoint_sig(method: &Method) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        fn #method_name #generics (
            self,
            #(#arg_decl),*
        ) -> testnumbat_wasm::types::ContractCall<Self::Api, <#ret_tok as testnumbat_wasm::io::EndpointResult>::DecodeAs>
        #generics_where
    };
    result
}
...
pub fn generate_proxy_deploy_sig(method: &Method) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        fn #method_name #generics (
            self,
            #(#arg_decl),*
        ) -> testnumbat_wasm::types::ContractDeploy<Self::Api>
        #generics_where
    };
    result
}
...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    let mut token_expr = quote! { testnumbat_wasm::types::TokenIdentifier::rewa(___api___.clone()) };
    let mut nonce_count = 0;
    let mut nonce_expr = quote! { 0u64 };
    let mut payment_count = 0;
    let mut payment_expr = quote! { testnumbat_wasm::types::BigUint::zero(___api___.clone()) };
    ...
    let sig = quote! {
        ...
        #msig {
            let (___api___, ___address___) =
                self.into_fields();
            let mut ___contract_deploy___ = testnumbat_wasm::types::new_contract_deploy(
                ___api___.clone(),
                ___address___,
            );
            #(#arg_push_snippets)*
...
pub fn proxy_trait(contract: &ContractTrait) -> proc_macro2::TokenStream {
    ...
    quote! {
        pub trait ProxyTrait:
            testnumbat_wasm::contract_base::ProxyObjBase
            + Sized
