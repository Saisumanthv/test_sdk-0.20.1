...
fn generate_key_snippet(key_args: &[MethodArgument], identifier: &str) -> proc_macro2::TokenStream {
    ...
    let key_appends: Vec<proc_macro2::TokenStream> = key_args
        ...
        .collect();
    quote! {
        let mut ___key___ = testnumbat_wasm::storage::StorageKey::<Self::Api>::new(
            self.raw_vm_api(),
            &#id_literal[..],
        );
        #(#key_appends)*
    ...
    match m.return_type.clone() {
        ...
        syn::ReturnType::Type(_, _ty) => {
            quote! {
                #msig {
                    #key_snippet
                    testnumbat_wasm::storage::storage_get(self.raw_vm_api(), &___key___)
                }
            }
        },
    }
...
pub fn generate_setter_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            testnumbat_wasm::storage::storage_set(self.raw_vm_api(), &___key___, & #pat);
        }
    }
}
    ...
    match m.return_type.clone() {
        ...
        syn::ReturnType::Type(_, ty) => {
            quote! {
                #msig {
                    #key_snippet
                    <#ty as testnumbat_wasm::storage::mappers::StorageMapper<Self::Api>>::new(
                        self.raw_vm_api(),
                        ___key___
                    )
                }
...
pub fn generate_is_empty_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            testnumbat_wasm::storage::storage_get_len(self.raw_vm_api(), &___key___) == 0
        }
    }
}
...
pub fn generate_clear_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            testnumbat_wasm::storage::storage_clear(self.raw_vm_api(), &___key___);
        }
    }
}
