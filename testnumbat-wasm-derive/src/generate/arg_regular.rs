    ...
    match &arg.ty {
        syn::Type::Reference(type_reference) => {
            ...
            if let syn::Type::Slice(slice_type) = &*type_reference.elem {
                ...
                quote! {
                    testnumbat_wasm::load_single_arg::<Self::Api, Box<[#slice_elem]>>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
                }
            } else {
                ...
                if let syn::Type::Path(syn::TypePath { path, .. }) = referenced_type {
                    if let Some(ident) = path.get_ident() {
                        if *ident == "str" {
                            ...
                            return quote! {
                                testnumbat_wasm::load_single_arg::<Self::Api, Box<str>>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
                            };
                        }
                    }
                }
                ...
                quote! {
                    testnumbat_wasm::load_single_arg::<Self::Api, #referenced_type>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
                }
            }
        },
        _ => {
            quote! {
                testnumbat_wasm::load_single_arg::<Self::Api, #arg_ty>(self.raw_vm_api(), #arg_index_expr, #arg_name_expr)
            }
        },
    }
}
...
pub fn generate_load_dyn_arg(
    ...
    match &arg.ty {
        syn::Type::Reference(type_reference) => {
            ...
            let referenced_type = &*type_reference.elem;
            quote! {
                let #pat: & #referenced_type = &testnumbat_wasm::load_dyn_arg(#loader_expr, #arg_name_expr);
            }
        },
        _ => {
            quote! {
                let #pat: #arg_ty = testnumbat_wasm::load_dyn_arg(#loader_expr, #arg_name_expr);
            }
        },
    }
}
