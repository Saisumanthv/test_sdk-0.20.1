...
pub fn generate_event_impl(m: &Method, event_identifier: &str) -> proc_macro2::TokenStream {
    ...
    let topic_push_snippets: Vec<proc_macro2::TokenStream> = topic_args
        ...
        .map(|arg| {
            ...
            quote! {
                testnumbat_wasm::log_util::serialize_event_topic(&mut ___topic_accumulator___, #topic_pat);
            }
        })
        .collect();
    let data_buffer_snippet = if let Some(data_arg) = data_arg {
        ...
        quote! {
            let ___data_buffer___ = testnumbat_wasm::log_util::serialize_log_data(self.raw_vm_api(), #data_pat);
        }
    } else {
        quote! {
            let ___data_buffer___ = testnumbat_wasm::types::ManagedBuffer::new(self.raw_vm_api());
        }
    };
    ...
    quote! {
        #msig {
            let mut ___topic_accumulator___ = testnumbat_wasm::log_util::event_topic_accumulator(
                self.raw_vm_api(),
                #event_identifier_literal,
            );
            #(#topic_push_snippets)*
            #data_buffer_snippet
            testnumbat_wasm::log_util::write_log(self.raw_vm_api(), &___topic_accumulator___, &___data_buffer___);
        }
    }
}
    ...
    let topic_conv_snippets: Vec<proc_macro2::TokenStream> = m
		...
		.map(|arg| {
			let result = if topic_index < nr_args_no_self {
				...
				let pat = &arg.pat;
				quote! {
					let data_vec = match testnumbat_wasm::testnumbat_codec::top_encode_to_vec(&#pat) {
