...
fn payable_snippet_for_metadata(
    ...
    match mpm {
        ...
        MethodPayableMetadata::Rewa => {
            ...
            let nonce_init = zero_nonce_init(payment_nonce_arg);
            quote! {
                let #payment_var_name = testnumbat_wasm::api::CallValueApi::require_rewa(&self.raw_vm_api());
                #token_init
                #nonce_init
            }
        },
        MethodPayableMetadata::SingleDcdtToken(token_identifier) => {
            ...
            quote! {
                let #payment_var_name = testnumbat_wasm::api::CallValueApi::require_dcdt(&self.raw_vm_api(), #token_literal);
                #token_init
                #nonce_init
            }
        },
        MethodPayableMetadata::AnyToken => {
            ...
            if payment_amount_arg.is_none() && payment_token_arg.is_none() {
                ...
                quote! {
                    let (#payment_var_name, #token_var_name) = testnumbat_wasm::api::CallValueApi::payment_token_pair(&self.raw_vm_api());
                    #nonce_init
                }
            }
        },
...
fn rewa_token_init(opt_arg: &Option<MethodArgument>) -> proc_macro2::TokenStream {
    if let Some(arg) = opt_arg {
        let pat = &arg.pat;
        quote! {
...
fn nonce_getter_init(opt_arg: &Option<MethodArgument>) -> proc_macro2::TokenStream {
    if let Some(arg) = opt_arg {
        let pat = &arg.pat;
        quote! {
            let #pat = self.call_value().dcdt_token_nonce();
        }
    } else {
        quote! {}
    }
