...
fn generate_abi_method_body(
    ...
    let supertrait_snippets: Vec<proc_macro2::TokenStream> = if is_contract_main {
        contract
			...
			.map(|supertrait| {
				...
				quote! {
					contract_abi.coalesce(<#module_path AbiProvider as testnumbat_wasm::contract_base::ContractAbiProvider>::abi());
				}
			})
			.collect()
    } else {
    ...
    quote! {
        let mut contract_abi = testnumbat_wasm::abi::ContractAbi{
            build_info: testnumbat_wasm::abi::BuildInfoAbi {
                contract_crate: testnumbat_wasm::abi::ContractCrateBuildAbi {
                    name: env!("CARGO_PKG_NAME"),
                    version: env!("CARGO_PKG_VERSION"),
                },
                framework: testnumbat_wasm::abi::FrameworkBuildAbi::create(),
            },
            docs: &[ #(#contract_docs),* ],
            name: #contract_name,
            constructor: None,
            endpoints: Vec::new(),
            type_descriptions: <testnumbat_wasm::abi::TypeDescriptionContainerImpl as testnumbat_wasm::abi::TypeDescriptionContainer>::new(),
        };
        #(#endpoint_snippets)*
        #(#supertrait_snippets)*
        contract_abi
