...
fn field_snippet(index: usize, field: &syn::Field) -> proc_macro2::TokenStream {
    ...
    quote! {
        field_descriptions.push(testnumbat_wasm::abi::StructFieldDescription {
            docs: &[ #(#field_docs),* ],
            name: #field_name_str,
...
pub fn type_abi_derive(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let type_description_impl = match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            quote! {
                fn provide_type_descriptions<TDC: testnumbat_wasm::abi::TypeDescriptionContainer>(accumulator: &mut TDC) {
                    let type_name = Self::type_name();
                    if !accumulator.contains_type(&type_name) {
                        accumulator.reserve_type_name(type_name.clone());
                        let mut field_descriptions = testnumbat_wasm::Vec::new();
                        #(#struct_field_snippets)*
                        accumulator.insert(
                            type_name.clone(),
                            testnumbat_wasm::abi::TypeDescription {
                                docs: &[ #(#type_docs),* ],
                                name: type_name,
                                contents: testnumbat_wasm::abi::TypeContents::Struct(field_descriptions),
                            },
                        );
                    }
                }
        ...
        syn::Data::Enum(data_enum) => {
            let enum_variant_snippets: Vec<proc_macro2::TokenStream> = data_enum
                ...
                .map(|(variant_index, variant)| {
                    ...
                    quote! {
                        ...
                        variant_descriptions.push(testnumbat_wasm::abi::EnumVariantDescription {
                            docs: &[ #(#variant_docs),* ],
            ...
            quote! {
                fn provide_type_descriptions<TDC: testnumbat_wasm::abi::TypeDescriptionContainer>(accumulator: &mut TDC) {
                    ...
                    if !accumulator.contains_type(&type_name) {
                        accumulator.reserve_type_name(type_name.clone());
                        let mut variant_descriptions = testnumbat_wasm::Vec::new();
                        ...
                        accumulator.insert(
                            ...
                            testnumbat_wasm::abi::TypeDescription {
                                docs: &[ #(#type_docs),* ],
                                name: type_name,
                                contents: testnumbat_wasm::abi::TypeContents::Enum(variant_descriptions),
                            },
                        );
                    }
                }
    ...
    let type_abi_impl = quote! {
        impl #impl_generics testnumbat_wasm::abi::TypeAbi for #name #ty_generics #where_clause {
            fn type_name() -> testnumbat_wasm::String {
                #name_str.into()
            }
