...
use testdenali::{Account, AddressKey, BlockInfo, NewAddress};
...
pub fn execute(
    ...
    for (address, account) in accounts.iter() {
        let storage = account
            ...
            .collect();
        let dcdt = if let Some(dcdt_map) = &account.dcdt {
            dcdt_map
                .iter()
                .map(|(k, v)| (k.value.clone(), v.value.clone()))
                .collect()
...
fn update_block_info(block_info: &mut CrateBlockInfo, testdenali_block_info: &testdenali::BlockInfo) {
    if let Some(u64_value) = &testdenali_block_info.block_timestamp {
        block_info.block_timestamp = u64_value.value;
    }
    if let Some(u64_value) = &testdenali_block_info.block_nonce {
        block_info.block_nonce = u64_value.value;
    }
    if let Some(u64_value) = &testdenali_block_info.block_epoch {
        block_info.block_epoch = u64_value.value;
    }
    if let Some(u64_value) = &testdenali_block_info.block_round {
        block_info.block_round = u64_value.value;
    }
    if let Some(bytes_value) = &testdenali_block_info.block_random_seed {
