...
use alloc::vec::Vec;
use testnumbat_wasm::api::{BigIntApi, Handle, ManagedBufferApi, StorageReadApi, StorageWriteApi};
...
impl StorageWriteApi for TxContext {
    fn storage_store_slice_u8(&self, key: &[u8], value: &[u8]) {
        ...
        if key.starts_with(&b"TESTNUMBAT"[..]) {
            std::panic::panic_any(TxPanic {
                status: 10,
                message: b"cannot write to storage under Testnumbat reserved key".to_vec(),
            });
        }
