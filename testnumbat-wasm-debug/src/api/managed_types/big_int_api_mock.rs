...
impl BigIntApi for TxContext {
    ...
    fn bi_sign(&self, x: Handle) -> testnumbat_wasm::api::Sign {
        let tx_output = self.tx_output_cell.borrow();
        let bi = tx_output.managed_types.big_int_map.get(x);
        match bi.sign() {
            num_bigint::Sign::Minus => testnumbat_wasm::api::Sign::Minus,
            num_bigint::Sign::NoSign => testnumbat_wasm::api::Sign::NoSign,
            num_bigint::Sign::Plus => testnumbat_wasm::api::Sign::Plus,
        }
    }
