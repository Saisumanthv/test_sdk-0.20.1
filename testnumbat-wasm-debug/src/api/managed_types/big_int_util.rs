...
use testnumbat_wasm::types::ManagedType;
use num_bigint::Sign;
use num_traits::Zero;
...
impl TxContext {
    ...
    ) -> testnumbat_wasm::types::BigUint<Self> {
        let mut tx_output = self.tx_output_cell.borrow_mut();
        let handle = tx_output
            .managed_types
            .big_int_map
            .insert_new_handle(value.into());
        testnumbat_wasm::types::BigUint::from_raw_handle(self.clone(), handle)
    }

    pub fn insert_new_big_uint_zero(&self) -> testnumbat_wasm::types::BigUint<Self> {
        self.insert_new_big_uint(num_bigint::BigUint::zero())
    }

    pub fn big_uint_value(&self, bu: &testnumbat_wasm::types::BigUint<Self>) -> num_bigint::BigUint {
